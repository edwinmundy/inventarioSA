// ==========================================
// SISTEMA DE INVENTARIO CON CATEGORÍAS DINÁMICAS V2
// ==========================================
...
// ==========================================
// SISTEMA DE CATEGORÍAS DINÁMICAS V2
// ==========================================
...
/**
 * Redirige a la página dinámica de categoría
 */
...
/**
 * Obtiene el ID de categoría de la URL actual
 */
...
/**
 * Carga una categoría en la página dinámica
 */
function cargarCategoriaDinamica() {
    ...
    // Guardar referencia para el botón de agregar
...
/**
 * Carga los items en el grid (reutilizable)
 */
...
/**
 * Crea una categoría - VERSIÓN SIMPLIFICADA SIN GENERAR HTML
 */
function crearCategoria(id, nombre, icono = '📦', descripcion = '') {
    ...
    // Validar ID válido (solo letras, números y guiones)
    if (!/^[a-z0-9-]+$/.test(id)) {
    ...
    // Inicializar datos vacíos para la nueva categoría
    guardarDatos(id, []);
    ...
    // Redirigir automáticamente a la nueva categoría
    setTimeout(() => {
        verCategoria(id);
    }, 800);
...
/**
 * Función mejorada para crear categoría desde el formulario
 */
function crearCategoriaCompleta(id, nombre, icono = '📦', descripcion = '') {
    return crearCategoria(id, nombre, icono, descripcion);
}
...
// ==========================================
// INICIALIZACIÓN CORREGIDA PARA GITHUB PAGES
// ==========================================
...
function inicializarDatos() {
    ...
    categorias.forEach(cat => {
        const key = obtenerStorageKey(cat.id);
    ...
    if (!cajerosExistentes) {
        // Primera vez: crear solo el admin
        console.log('Primera inicialización: creando admin por defecto...');
        localStorage.setItem(cajerosKey, JSON.stringify(DATOS_DEFAULT.cajeros));
    }
    // Si ya existen cajeros, NO tocarlos (incluso si el array está vacío intencionalmente)
...
function asegurarAdminExiste() {
    // Solo usar en login.html para verificar que hay al menos un admin
    // NO usar en páginas principales para no sobreescribir
    ...
    const adminExiste = cajeros.find(c => c && c.codigo === 'ADMIN');
    
    if (!adminExiste && cajeros.length === 0) {
        // Solo crear admin si no existe Y no hay otros cajeros
...
function actualizarNavegacion() {
    ...
    categorias.forEach(cat => {
        // Detectar si estamos en categoria.html viendo esta categoría específica
        const esActiva = paginaActual === 'categoria.html' && categoriaActual === cat.id;
        const claseActiva = esActiva ? 'active' : '';
        
        html += `
            <a href="categoria.html?cat=${cat.id}" class="nav-btn ${claseActiva}">
    ...
    html += `
        ...
        <a href="inventario.html" class="nav-btn ${paginaActual === 'inventario.html' ? 'active' : ''}">📊 Inventario General</a>
        <a href="categorias.html" class="nav-btn ${paginaActual === 'categorias.html' ? 'active' : ''}">⚙️ Categorías</a>
    `;
...
// Función legacy para compatibilidad con páginas antiguas
...
// ==========================================
// INVENTARIO GENERAL Y DASHBOARD
// ==========================================
...
function actualizarDashboard() {
    ...
    categorias.forEach(cat => {
        ...
        const itemsBajos = items.filter(i => i.cantidad <= i.minimo).length;
        
        html += `
            <div class="stat-card" onclick="verCategoria('${cat.id}')" style="cursor: pointer;">
                <h3>${cat.icono} ${cat.nombre}</h3>
                <div class="stat-number">${totalItems}</div>
                <small style="color: ${itemsBajos > 0 ? 'var(--danger)' : 'var(--success)'}">
                    ${itemsBajos > 0 ? `⚠️ ${itemsBajos} con stock bajo` : '✓ Todo OK'}
                </small>
            </div>
        `;
    });
    ...
    const cajeros = obtenerDatos('cajeros');
    html += `
        <div class="stat-card" onclick="location.href='cajeros.html'" style="cursor: pointer;">
            <h3>👤 Cajeros</h3>
            <div class="stat-number">${cajeros.length}</div>
            <small>Activos</small>
        </div>
    `;
...
function exportarCategoria(categoria) {
    const items = obtenerDatos(categoria);
    const cat = obtenerCategorias().find(c => c.id === categoria);
    ...
    const fecha = new Date().toISOString().split('T')[0];
    XLSX.writeFile(wb, `inventario_${categoria}_${fecha}.xlsx`);
    
    mostrarNotificacion(`Inventario ${cat.nombre} exportado a Excel`);
}
...
window.onclick = function(event) {
    ...
    const modalCat = document.getElementById('modal-editar-cat');
    if (event.target === modalCat) {
        if (typeof cerrarModalCat === 'function') {
            cerrarModalCat();
        }
    }
}
